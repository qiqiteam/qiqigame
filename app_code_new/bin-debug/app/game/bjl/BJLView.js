...
var BJLView = (function (_super) {
    ...
    function BJLView() {
        ...
        _this.multipleList = [0, 0, 0, 0, 0, 0]; //倍数表
        _this.istzxzDh = false;
        //----------------------------------------------------------------
    ...
    BJLView.prototype.childrenCreated = function () {
        ...
        // this.grpCard_0_0.source = 'qznn_card_' + 403;
        // this.grpCard_0_1.source = 'qznn_card_' + 409;
        // this.puke_0_0.source = 'qznn_card_' + 409;
        // this.puke_0_0.visible = true;
        // this.puke_0_1.source = 'qznn_card_' + 403;
        // this.puke_0_1.visible = true;
        // this._bei_puke_0_0.source = 'qznn_card_' + 409;
        // this._bei_puke_0_1.source = 'qznn_card_' + 403;
    ...
    BJLView.prototype.initData = function () {
        ...
        this._bei_puke_0_2.source = '';
        this._bei_puke_1_2.source = '';
        /*
                for (let i = 0; i < 2; i++) {
                    for (let j = 0; j < 2; j++) {
                        let _juXing = new egret.Shape;
                        _juXing.graphics.beginFill(0xff0000);
                        _juXing.graphics.drawRect(0, 0, 170, 255);
                        _juXing.graphics.endFill();
                        _juXing.x = this["_bei_puke_" + i + "_" + j].x;
                        _juXing.y = this["_bei_puke_" + i + "_" + j].y;
                        this.addChild(_juXing);
                        this["_bei_puke_" + i + "_" + j].mask = _juXing;
                        this["puke_" + i + "_" + j].mask = _juXing;
                    }
                }
        */
    };
    ...
    /**个人限红通知 */
    BJLView.prototype.UpdateLimitItem = function (data) {
        xlLib.TipsUtils.showFloatWordTips("个人限红通知!");
    };
    /**游戏状态 */
    BJLView.prototype.GameStatus = function (data) {
        ...
        switch (data._obj.roomStatus) {
            ...
            case 5:
                ...
                break; //发牌（动画）
            case 3:
                this.xianjiamipai(data);
                break; //闲家咪牌（发牌）
            case 6:
                this.zhuangjiamipai(data);
                break; //庄家咪牌（发牌）
            case 7:
                this.xianjiaBupai(data);
                break; //闲家补牌
            case 8:
                this.zhuangjiaBupai(data);
                break; //庄家补牌
            case 4:
                this.onBeginBteonBack(data);
                break; //派奖（结算）
            case 9:
                this.Zhuangbeizho(data);
                break; //第二轮开始
        }
    };
    /**第二轮开始 */
    BJLView.prototype.Zhuangbeizho = function (data) {
        this.startCountDown(data._obj.seconds);
        this.resetGame();
    };
    /**(筹码) */
    BJLView.prototype.SendJetton = function (data) {
        for (var i = 0; i < data._obj.betrules.length; i++) {
    ...
    /**更新下注通知(所有人) */
    BJLView.prototype.OnBetUpdate = function (data) {
    ...
    /**闲家咪牌 */
    BJLView.prototype.xianjiamipai = function (data) {
        this.istzxzDh = false;
        this.startCountDown(data._obj.seconds);
    ...
    /**庄家咪牌 */
    BJLView.prototype.zhuangjiamipai = function (data) {
        this.startCountDown(data._obj.seconds);
        ...
        this._bei_puke_1_0.visible = false;
        this._bei_puke_1_1.visible = false;
    };
    /**闲家补牌 */
    BJLView.prototype.xianjiaBupai = function (data) {
        this.startCountDown(data._obj.seconds);
        this.dianshu_img_0.source = 'baccarat_game_point_' + data._obj.playersce + '_png';
        this.xian_tishi.visible = true;
    ...
    /**庄家补牌 */
    BJLView.prototype.zhuangjiaBupai = function (data) {
        this.startCountDown(data._obj.seconds);
        this.dianshu_img_1.source = 'baccarat_game_point_' + data._obj.bankersce + '_png';
        this.xian_tishi.visible = false;
    ...
    /**当前牌局的 庄赢 闲赢  和 */
    BJLView.prototype.bofangEffect = function (data) {
        this.startCountDown(data._obj.seconds);
        if (data._obj.base == 1) {
            this.addjiesuan_1Effect();
        }
    ...
    /**请求玩家列表*/
    BJLView.prototype.wanjialiebiao = function (sendstr) {
        var senddata = {
            userid: UserInfo.getInstance().uid,
            token: UserInfo.getInstance().token,
        };
    ...
    /**设置金币最多的20个玩家数据 */
    BJLView.prototype.initGameIconList = function (data) {
        var dataArr = [];
        var otherplayer;
        for (var i = 0; i < data._obj.subList.length; i++) {
    ...
    /**通知类型 状态 */
    BJLView.prototype.TimelyNotify = function (data) {
    ...
    /**踢出房间 */
    BJLView.prototype.tichufj = function () {
        xlLib.SoundMgr.instance.stopBgMusic();
        var musicBg = ["hall_bg_mp3"];
        xlLib.SoundMgr.instance.playBgMusic(musicBg);
        xlLib.SceneMgr.instance.changeScene(Lobby);
    ...
    /**胜利 */
    BJLView.prototype.addjiesuan_1Effect = function () {
        if (!this.jiesuan_1) {
    ...
    /**胜利 */
    BJLView.prototype.addjiesuan_2Effect = function () {
        if (!this.jiesuan_2) {
    ...
    /**胜利 */
    BJLView.prototype.addjiesuan_3Effect = function () {
        if (!this.jiesuan_3) {
    ...
    /**下注开始 */
    BJLView.prototype.xiazhu_Effect = function () {
        if (!this.xiazhu) {
    ...
    /**停止下注 */
    BJLView.prototype.tingzhi_Effect = function () {
        if (!this.tingzhi) {
    ...
    /**自己点击跟注 (丢筹码动作) */
    BJLView.prototype.onGenZhuClick = function (num, index) {
    ...
    /**其他玩家跟注 (丢筹码动作) */
    BJLView.prototype.onPlayerGenZhu = function (num, index) {
        var p = new egret.Point();
        p.x = 50;
        p.y = 604;
        this.showCoins(p, num, index);
    ...
    /**筹码池分数(总分)  */
    BJLView.prototype.onJettongrade = function (num, index) {
    ...
    /**筹码池分数(自己)  */
    BJLView.prototype.onplaygrade = function (num, index) {
    ...
    //筹码选择
    BJLView.prototype.onTouchBet = function (index) {
        ...
        switch (index) {
            case 1:
                this.xuanzho.x = 337.78;
                break;
            case 2:
                this.xuanzho.x = 475.16;
                break;
            case 3:
                this.xuanzho.x = 603.77;
                break;
            case 4:
                this.xuanzho.x = 736.77;
                break;
            case 5:
                this.xuanzho.x = 866.1;
                break;
            case 6:
                this.xuanzho.x = 997.77;
                break;
        }
    };
    //设置彩金
    ...
    */
    ...
    /**发牌动画 */
    BJLView.prototype.fapai = function () {
        var card = this['grpCard_' + this.fapai_0 + '_' + this.fapai_1];
        card.x = 713.5;
        card.y = 100;
    ...
    BJLView.prototype.fapai1 = function () {
        if (this.fapai_0 == 1) {
            this.fapai_0 = 0;
            this.fapai_1++;
        }
    ...
    /**补牌动画 */
    BJLView.prototype.fapai2 = function (index, num) {
        var card = this['grpCard_' + index + '_2'];
        card.x = 713.5;
        card.y = 100;
