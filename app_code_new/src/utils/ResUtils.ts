    ...
    export class ResUtils {
        ...
        */
        public static getRes(source: string, compFunc?: Function, thisObject?: any, type?: string): void {
            ...
            function onSoundGetRes(data: any): void {
                self._resRequestSoundLoadTimes[source] = 0;
                var sound: egret.Sound = <egret.Sound>data;
            ...
            //             // var item: RES.ResourceItem = resConfig.getResourceItem(source);

            //             // var url: string = RES.getVersionController().getVirtualUrl(item.url);
            ...
            if (type == egret.URLLoaderDataFormat.SOUND) {
                ...
                RES.getResAsync(source, onSoundGetRes, this);
                // var loader: egret.URLLoader = new egret.URLLoader();
                // //设置加载方式为声音
                // loader.dataFormat = egret.URLLoaderDataFormat.SOUND;
                // //添加加载完成侦听
                ...
                //     loader.load(request);
                // } else {
                //     Console.error("找不到将要加载的资源：" + source + "\t" + url);
                // }
            } else {
        ...
        public static clearResConfigByConfigName(arr: string[]): void{
            for(var index:number;index<arr.length;index++){
                RES.destroyRes(arr[index]);
            }
        ...
        public static clearResConfigByGroupName(groupArr: string[]): void{
            this.clearResConfigByConfigName(groupArr);
            // var configRes = RES["configInstance"];
